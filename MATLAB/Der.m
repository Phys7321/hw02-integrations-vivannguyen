...
if ~isa(F,'function_handle')
    if isnumeric(F)
      xc = x(1:end-1); % xc = x(1:end-1) method for data depends on how you assign dy to xc
      dy = diff(F)./diff(x); 
      varargin{1} = 'data';
    else
...
switch varargin{1}
    ...
    case 'cd'
        ...
        xc = x(1:n-1);
    case 'cd_cubic'
        dy = ((1/24).*F(x(1:n-1)-(3/2).*dx) - (27/24).*F(x(1:n-1)-0.5*dx) + (27/24).*F(x(1:n-1)+0.5*dx) - (1/24).*F(x(1:n-1)+(3/2).*dx))./dx;
        xc = x(1:n-1);       
    case 'ed'
